...
# Fallback model(s), tried in order when the primary exhausts its retries.
# Supports a comma-separated list in .env, e.g.:
#   AI_MODEL_FALLBACK=minimax/minimax-m3:free,z-ai/glm-5.2:free,thinkingmachines/inkling:free
# Only after ALL models fail does the run fall back to the pure-Python indicator
# decision. AI_MODEL_FALLBACK is kept as a plain string for backwards compat;
# ai_decision._complete() reads AI_MODEL_FALLBACKS (list) for the full chain.
AI_MODEL_FALLBACK = os.getenv("AI_MODEL_FALLBACK", "").strip()
# Parsed list of fallback models (primary is NOT included here).
AI_MODEL_FALLBACKS: list[str] = [
    m.strip() for m in AI_MODEL_FALLBACK.split(",")
    if m.strip() and m.strip() != AI_MODEL
]
...
def check_config_warnings() -> list[str]:
    ...
    if any(m == AI_MODEL for m in AI_MODEL_FALLBACKS):
        warnings.append("AI_MODEL_FALLBACK contains the primary AI_MODEL — that entry "
                        "is ignored; remove it or set a different fallback model")
