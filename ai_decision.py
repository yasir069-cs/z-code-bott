...
def _complete(messages: list, parse=None, deadline: Optional[float] = None,
    ...
    # Build the full model sequence: primary first, then all comma-separated
    # fallbacks from config.AI_MODEL_FALLBACKS (parsed from AI_MODEL_FALLBACK
    # in .env). A single-model .env still works — the list will just be empty.
    models = [config.AI_MODEL] + list(getattr(config, "AI_MODEL_FALLBACKS", []))
